...
func VerifyFrameSignature(packet *FrameSignaturePacket) (*farcasterpb.FrameActionBody, *farcasterpb.Message, ed25519.PublicKey, error) {
	// Decode the message bytes from hex
	messageBytes, err := hex.DecodeString(packet.TrustedData.MessageBytes)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to decode message bytes: %w", err)
	}

	msg := farcasterpb.Message{}
	if err := proto.Unmarshal(messageBytes, &msg); err != nil {
		return nil, nil, nil, fmt.Errorf("failed to unmarshal Message: %w", err)
	}
	log.Debugf("farcaster signed message: %s", log.FormatProto(&msg))

	if msg.Data == nil {
		return nil, nil, nil, fmt.Errorf("invalid message data")
	}
	if msg.SignatureScheme != farcasterpb.SignatureScheme_SIGNATURE_SCHEME_ED25519 {
		return nil, nil, nil, fmt.Errorf("invalid signature scheme")
	}
	if msg.Data.Type != farcasterpb.MessageType_MESSAGE_TYPE_FRAME_ACTION {
		return nil, nil, nil, fmt.Errorf("invalid message type, got %s", msg.Data.Type.String())
	}
	var pubkey ed25519.PublicKey = msg.GetSigner()

	if pubkey == nil {
		return nil, nil, nil, fmt.Errorf("signer is nil")
	}

	// Verify the hash and signature
	msgDataBytes, err := proto.Marshal(msg.Data)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to marshal message data: %w", err)
	}
	...
	if !bytes.Equal(msg.Hash, hashed) {
		return nil, nil, nil, fmt.Errorf("hash mismatch (got %x, expected %x)", hashed, msg.Hash)
	}
	...
	if actionBody == nil {
		return nil, nil, nil, fmt.Errorf("invalid action body")
	}

	return actionBody, &msg, pubkey, nil
}
